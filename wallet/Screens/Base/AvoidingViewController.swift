    ...
    func notifyKeyboard(_ notification: NSNotification) {
        ...
        UIView.animate(withDuration: duration, delay: TimeInterval(0), options: animationCurve, animations: {
            if let keyboardView = endFrame, keyboardView.origin.y < UIScreen.main.bounds.size.height {
                ...
                self.fastenBottomConstraint?.constant = UIDevice.current.iPhoneX ? keyboardView.size.height + self.fastenInitialOffset - 32 + tabBarOffset : keyboardView.size.height + self.fastenInitialOffset + tabBarOffset
            } else {
